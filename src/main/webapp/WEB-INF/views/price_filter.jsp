		...
		<c:forEach var="item" begin="1" end="${paginateInfo.totalPage}"
			...
			<ul class="store-pagination">
				<c:if test="${(loop.index)==paginateInfo.currentPage }">
					...
					<li class="active"><a
						href="products-price?priceRange&page=${loop.index}">${loop.index}</li>
				</c:if>
				<c:if test="${(loop.index)!=paginateInfo.currentPage}">
					<li><a
						href="products-price?priceRange&page=${loop.index}">${loop.index}</a></li>
				</c:if>
		</c:forEach>
		<!-- Nút điều hướng đến trang tiếp theo -->
		<c:if test="${paginateInfo.currentPage < paginateInfo.totalPage}">
			<li><a
				href="products-price?priceRange&page=${paginateInfo.currentPage + 1}">
