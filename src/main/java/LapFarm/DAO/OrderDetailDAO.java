...
public class OrderDetailDAO {
	...
	public List<OrderDetailDTO> getOrderDetailById(int id) {
		...
		List<Integer> productIds = orderDetails.stream().map(orderDetail -> orderDetail.getProduct().getIdProduct())
				.distinct().toList();
		...
		List<OrderDetailDTO> orderDetailDTOs = orderDetails.stream().map(orderDetail -> {
			...
			ProductDTO productDTO = new ProductDTO(product.getIdProduct(), product.getNameProduct(),
					...
					product.getSalePrice(), product.getState(),
					product.getImages() != null && !product.getImages().isEmpty()
							? product.getImages().get(0).getImageUrl()
							: null // Lấy ảnh đầu tiên nếu có
			);

			// Chuyển đổi thông tin từ OrderDetailsEntity sang OrderDetailDTO
			return new OrderDetailDTO(orderDetail.getOrder().getIdOrder(), productDTO, orderDetail.getQuantity(),
	...
	public int countSalesByProductId(int productId) {
		...
		// Truy vấn các OrderDetailsEntity có trạng thái "Hoàn thành" và idProduct tương
		// ứng
